...
public class MstDriverGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all mstDrivers").get("/api/mst-drivers").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new mstDriver")
                        .post("/api/mst-drivers")
                        ...
                        .body(
                            StringBody(
                                ...
                                ", \"vehicleDetails\": \"SAMPLE_TEXT\"" +
                                "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_mstDriver_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created mstDriver").get("${new_mstDriver_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created mstDriver").delete("${new_mstDriver_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the MstDriver entity").exec(scn);
