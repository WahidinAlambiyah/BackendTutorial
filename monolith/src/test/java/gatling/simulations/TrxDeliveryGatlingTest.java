...
public class TrxDeliveryGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all trxDeliveries").get("/api/trx-deliveries").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new trxDelivery")
                        .post("/api/trx-deliveries")
                        ...
                        .body(
                            StringBody(
                                ...
                                ", \"assignedDriver\": \"SAMPLE_TEXT\"" +
                                ", \"estimatedDeliveryTime\": \"2020-01-01T00:00:00.000Z\"" +
                                "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_trxDelivery_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created trxDelivery").get("${new_trxDelivery_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created trxDelivery").delete("${new_trxDelivery_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the TrxDelivery entity").exec(scn);
