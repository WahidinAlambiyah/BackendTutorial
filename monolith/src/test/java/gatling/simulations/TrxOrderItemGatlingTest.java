...
public class TrxOrderItemGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all trxOrderItems").get("/api/trx-order-items").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new trxOrderItem")
                        .post("/api/trx-order-items")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"quantity\": 0" + ", \"price\": 0" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_trxOrderItem_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created trxOrderItem").get("${new_trxOrderItem_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created trxOrderItem").delete("${new_trxOrderItem_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the TrxOrderItem entity").exec(scn);
