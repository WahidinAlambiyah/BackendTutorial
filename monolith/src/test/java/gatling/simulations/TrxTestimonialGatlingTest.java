...
public class TrxTestimonialGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all trxTestimonials").get("/api/trx-testimonials").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new trxTestimonial")
                        .post("/api/trx-testimonials")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_trxTestimonial_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created trxTestimonial").get("${new_trxTestimonial_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created trxTestimonial").delete("${new_trxTestimonial_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the TrxTestimonial entity").exec(scn);
