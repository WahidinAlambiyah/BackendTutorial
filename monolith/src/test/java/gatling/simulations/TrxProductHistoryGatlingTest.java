...
public class TrxProductHistoryGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all trxProductHistories")
                    .get("/api/trx-product-histories")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new trxProductHistory")
                        .post("/api/trx-product-histories")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" + "\"oldPrice\": 0" + ", \"newPrice\": 0" + ", \"changeDate\": \"2020-01-01T00:00:00.000Z\"" + "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_trxProductHistory_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created trxProductHistory").get("${new_trxProductHistory_url}").headers(headers_http_authenticated)
                    ).pause(10)
                )
                .exec(http("Delete created trxProductHistory").delete("${new_trxProductHistory_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the TrxProductHistory entity").exec(scn);
