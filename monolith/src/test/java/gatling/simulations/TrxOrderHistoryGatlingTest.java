...
public class TrxOrderHistoryGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all trxOrderHistories")
                    .get("/api/trx-order-histories")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new trxOrderHistory")
                        .post("/api/trx-order-histories")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_trxOrderHistory_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(http("Get created trxOrderHistory").get("${new_trxOrderHistory_url}").headers(headers_http_authenticated)).pause(
                        10
                    )
                )
                .exec(http("Delete created trxOrderHistory").delete("${new_trxOrderHistory_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the TrxOrderHistory entity").exec(scn);
