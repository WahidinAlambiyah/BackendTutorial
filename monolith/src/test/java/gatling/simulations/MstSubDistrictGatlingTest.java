...
public class MstSubDistrictGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all mstSubDistricts").get("/api/mst-sub-districts").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new mstSubDistrict")
                        .post("/api/mst-sub-districts")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_mstSubDistrict_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created mstSubDistrict").get("${new_mstSubDistrict_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created mstSubDistrict").delete("${new_mstSubDistrict_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the MstSubDistrict entity").exec(scn);
