...
public class MstPostalCodeGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all mstPostalCodes").get("/api/mst-postal-codes").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new mstPostalCode")
                        .post("/api/mst-postal-codes")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"code\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_mstPostalCode_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created mstPostalCode").get("${new_mstPostalCode_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created mstPostalCode").delete("${new_mstPostalCode_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the MstPostalCode entity").exec(scn);
