...
public class TrxCouponGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all trxCoupons").get("/api/trx-coupons").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new trxCoupon")
                        .post("/api/trx-coupons")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_trxCoupon_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created trxCoupon").get("${new_trxCoupon_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created trxCoupon").delete("${new_trxCoupon_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the TrxCoupon entity").exec(scn);
