 ...
 */
public class StockGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all stocks").get("/api/stocks").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new stock")
                        .post("/api/stocks")
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created stock").get("${new_stock_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created stock").delete("${new_stock_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Stock entity").exec(scn);
