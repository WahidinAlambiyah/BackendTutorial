...
public class MstJobGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all mstJobs").get("/api/mst-jobs").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new mstJob")
                        .post("/api/mst-jobs")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"jobTitle\": \"SAMPLE_TEXT\"" + ", \"minSalary\": 0" + ", \"maxSalary\": 0" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_mstJob_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created mstJob").get("${new_mstJob_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created mstJob").delete("${new_mstJob_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the MstJob entity").exec(scn);
