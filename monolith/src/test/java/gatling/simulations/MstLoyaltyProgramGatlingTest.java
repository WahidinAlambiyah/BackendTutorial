...
public class MstLoyaltyProgramGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all mstLoyaltyPrograms")
                    .get("/api/mst-loyalty-programs")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new mstLoyaltyProgram")
                        .post("/api/mst-loyalty-programs")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"pointsEarned\": 0" + ", \"membershipTier\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_mstLoyaltyProgram_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created mstLoyaltyProgram").get("${new_mstLoyaltyProgram_url}").headers(headers_http_authenticated)
                    ).pause(10)
                )
                .exec(http("Delete created mstLoyaltyProgram").delete("${new_mstLoyaltyProgram_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the MstLoyaltyProgram entity").exec(scn);
