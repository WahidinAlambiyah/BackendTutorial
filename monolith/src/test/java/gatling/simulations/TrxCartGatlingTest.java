...
public class TrxCartGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all trxCarts").get("/api/trx-carts").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new trxCart")
                        .post("/api/trx-carts")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"totalPrice\": 0" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_trxCart_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created trxCart").get("${new_trxCart_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created trxCart").delete("${new_trxCart_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the TrxCart entity").exec(scn);
