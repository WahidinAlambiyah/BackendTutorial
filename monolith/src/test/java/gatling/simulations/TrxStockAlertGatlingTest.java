...
public class TrxStockAlertGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all trxStockAlerts").get("/api/trx-stock-alerts").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new trxStockAlert")
                        .post("/api/trx-stock-alerts")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"alertThreshold\": 0" + ", \"currentStock\": 0" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_trxStockAlert_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created trxStockAlert").get("${new_trxStockAlert_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created trxStockAlert").delete("${new_trxStockAlert_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the TrxStockAlert entity").exec(scn);
