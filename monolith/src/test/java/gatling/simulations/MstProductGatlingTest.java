...
public class MstProductGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all mstProducts").get("/api/mst-products").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new mstProduct")
                        .post("/api/mst-products")
                        ...
                        .body(
                            StringBody(
                                ...
                                ", \"quantity\": 0" +
                                ", \"barcode\": \"SAMPLE_TEXT\"" +
                                ", \"unitSize\": \"SAMPLE_TEXT\"" +
                                "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_mstProduct_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created mstProduct").get("${new_mstProduct_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created mstProduct").delete("${new_mstProduct_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the MstProduct entity").exec(scn);
