...
public class MstDepartmentGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all mstDepartments").get("/api/mst-departments").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new mstDepartment")
                        .post("/api/mst-departments")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"departmentName\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_mstDepartment_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created mstDepartment").get("${new_mstDepartment_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created mstDepartment").delete("${new_mstDepartment_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the MstDepartment entity").exec(scn);
