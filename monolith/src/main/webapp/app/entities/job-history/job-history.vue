<template>
  <div>
    <h2 id="page-heading" data-cy="JobHistoryHeading">
      <span v-text="t$('monolithApp.jobHistory.home.title')" id="job-history-heading"></span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="t$('monolithApp.jobHistory.home.refreshListLabel')"></span>
        </button>
        <router-link :to="{ name: 'JobHistoryCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="t$('monolithApp.jobHistory.home.createLabel')"></span>
          </button>
        </router-link>
      </div>
    </h2>
        ...
        <form name="searchForm" class="form-inline" v-on:submit.prevent="search(currentSearch)">
          <div class="input-group w-100 mt-3">
            <input
              ...
              name="currentSearch"
              id="currentSearch"
              v-bind:placeholder="t$('monolithApp.jobHistory.home.search')"
    ...
    <div class="alert alert-warning" v-if="!isFetching && jobHistories && jobHistories.length === 0">
      <span v-text="t$('monolithApp.jobHistory.home.notFound')"></span>
    </div>
      ...
      <table class="table table-striped" aria-describedby="jobHistories">
        <thead>
          <tr>
            ...
            <th scope="row" v-on:click="changeOrder('startDate')">
              <span v-text="t$('monolithApp.jobHistory.startDate')"></span>
            ...
            <th scope="row" v-on:click="changeOrder('endDate')">
              <span v-text="t$('monolithApp.jobHistory.endDate')"></span>
            ...
            <th scope="row" v-on:click="changeOrder('language')">
              <span v-text="t$('monolithApp.jobHistory.language')"></span>
            ...
            <th scope="row" v-on:click="changeOrder('job.id')">
              <span v-text="t$('monolithApp.jobHistory.job')"></span>
            ...
            <th scope="row" v-on:click="changeOrder('department.id')">
              <span v-text="t$('monolithApp.jobHistory.department')"></span>
            ...
            <th scope="row" v-on:click="changeOrder('employee.id')">
              <span v-text="t$('monolithApp.jobHistory.employee')"></span>
        ...
        <tbody>
          <tr v-for="jobHistory in jobHistories" :key="jobHistory.id" data-cy="entityTable">
            ...
            <td v-text="t$('monolithApp.Language.' + jobHistory.language)"></td>
            <td>
              <div v-if="jobHistory.job">
                <router-link :to="{ name: 'MstJobView', params: { mstJobId: jobHistory.job.id } }">{{ jobHistory.job.id }}</router-link>
              </div>
            </td>
            <td>
              <div v-if="jobHistory.department">
                <router-link :to="{ name: 'MstDepartmentView', params: { mstDepartmentId: jobHistory.department.id } }">{{
            ...
            <td>
              <div v-if="jobHistory.employee">
                <router-link :to="{ name: 'MstEmployeeView', params: { mstEmployeeId: jobHistory.employee.id } }">{{
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <template #modal-title>
        <span id="monolithApp.jobHistory.delete.question" data-cy="jobHistoryDeleteDialogHeading" v-text="t$('entity.delete.title')"></span>
      </template>
      <div class="modal-body">
        <p id="jhi-delete-jobHistory-heading" v-text="t$('monolithApp.jobHistory.delete.question', { id: removeId })"></p>
      </div>
