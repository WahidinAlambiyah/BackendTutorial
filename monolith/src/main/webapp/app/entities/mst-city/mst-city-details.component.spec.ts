/* tslint:disable max-line-length */
import { vitest } from 'vitest';
import { shallowMount, type MountingOptions } from '@vue/test-utils';
import sinon, { type SinonStubbedInstance } from 'sinon';
import { type RouteLocation } from 'vue-router';

import MstCityDetails from './mst-city-details.vue';
import MstCityService from './mst-city.service';
import AlertService from '@/shared/alert/alert.service';

type MstCityDetailsComponentType = InstanceType<typeof MstCityDetails>;

let route: Partial<RouteLocation>;
const routerGoMock = vitest.fn();

vitest.mock('vue-router', () => ({
  useRoute: () => route,
  useRouter: () => ({ go: routerGoMock }),
}));

const mstCitySample = { id: 123 };

describe('Component Tests', () => {
  let alertService: AlertService;

  afterEach(() => {
    vitest.resetAllMocks();
  });

  describe('MstCity Management Detail Component', () => {
    let mstCityServiceStub: SinonStubbedInstance<MstCityService>;
    let mountOptions: MountingOptions<MstCityDetailsComponentType>['global'];

    beforeEach(() => {
      route = {};
      mstCityServiceStub = sinon.createStubInstance<MstCityService>(MstCityService);

      alertService = new AlertService({
        i18n: { t: vitest.fn() } as any,
        bvToast: {
          toast: vitest.fn(),
        } as any,
      });

      mountOptions = {
        stubs: {
          'font-awesome-icon': true,
          'router-link': true,
        },
        provide: {
          alertService,
          mstCityService: () => mstCityServiceStub,
        },
      };
    });

    describe('Navigate to details', () => {
      it('Should call load all on init', async () => {
        // GIVEN
        mstCityServiceStub.find.resolves(mstCitySample);
        route = {
          params: {
            mstCityId: '' + 123,
          },
        };
        const wrapper = shallowMount(MstCityDetails, { global: mountOptions });
        const comp = wrapper.vm;
        // WHEN
        await comp.$nextTick();

        // THEN
        expect(comp.mstCity).toMatchObject(mstCitySample);
      });
    });

    describe('Previous state', () => {
      it('Should go previous state', async () => {
        mstCityServiceStub.find.resolves(mstCitySample);
        const wrapper = shallowMount(MstCityDetails, { global: mountOptions });
        const comp = wrapper.vm;
        await comp.$nextTick();

        comp.previousState();
        await comp.$nextTick();

        expect(routerGoMock).toHaveBeenCalledWith(-1);
      });
    });
  });
});
