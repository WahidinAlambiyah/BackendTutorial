...
public class TrxEventResource {
    ...
    public Flux<TrxEventDTO> getAllTrxEvents(TrxEventCriteria criteria) {
        log.debug("REST request to get TrxEvents by criteria: {}", criteria);
        return trxEventService.findByCriteria(criteria);
    }
     ...
     */
    @GetMapping("/_search")
    public Mono<List<TrxEventDTO>> searchTrxEvents(@RequestParam("query") String query) {
        log.debug("REST request to search TrxEvents for query {}", query);
        try {
            return trxEventService.search(query).collectList();
