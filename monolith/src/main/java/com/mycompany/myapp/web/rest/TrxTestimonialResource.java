...
public class TrxTestimonialResource {
    ...
    public Flux<TrxTestimonialDTO> getAllTrxTestimonials(TrxTestimonialCriteria criteria) {
        log.debug("REST request to get TrxTestimonials by criteria: {}", criteria);
        return trxTestimonialService.findByCriteria(criteria);
    }
     ...
     */
    @GetMapping("/_search")
    public Mono<List<TrxTestimonialDTO>> searchTrxTestimonials(@RequestParam("query") String query) {
        log.debug("REST request to search TrxTestimonials for query {}", query);
        try {
            return trxTestimonialService.search(query).collectList();
