...
public class PostalCodeGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all postalCodes").get("/api/postal-codes").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new postalCode")
                        .post("/api/postal-codes")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"code\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_postalCode_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created postalCode").get("${new_postalCode_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created postalCode").delete("${new_postalCode_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the PostalCode entity").exec(scn);
