 ...
 */
public class ServiceGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all services").get("/api/services").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new service")
                        .post("/api/services")
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created service").get("${new_service_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created service").delete("${new_service_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Service entity").exec(scn);
