...
public class EmployeeResource {
    ...
    public Mono<ResponseEntity<List<Employee>>> getAllEmployees(
        ...
        if ("jobhistory-is-null".equals(filter)) {
            ...
            return employeeRepository.findAllWhereJobHistoryIsNull().collectList().map(ResponseEntity::ok);
        }
    ...
    public Mono<ResponseEntity<Void>> deleteEmployee(@PathVariable("id") Long id) {
        ...
        return employeeRepository
            .deleteById(id)
            .then(employeeSearchRepository.deleteById(id))
            .then(
                Mono.just(
     ...
     */
    @GetMapping("/_search")
    public Mono<ResponseEntity<Flux<Employee>>> searchEmployees(
        @RequestParam("query") String query,
        @org.springdoc.core.annotations.ParameterObject Pageable pageable,
        ServerHttpRequest request
    ) {
        log.debug("REST request to search for a page of Employees for query {}", query);
        return employeeSearchRepository
            .count()
            ...
            .map(headers -> ResponseEntity.ok().headers(headers).body(employeeSearchRepository.search(query, pageable)));
    }
}
