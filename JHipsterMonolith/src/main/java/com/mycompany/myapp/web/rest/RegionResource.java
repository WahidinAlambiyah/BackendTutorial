...
public class RegionResource {
    ...
    public Mono<List<Region>> getAllRegions(@RequestParam(name = "filter", required = false) String filter) {
        if ("country-is-null".equals(filter)) {
            log.debug("REST request to get all Regions where country is null");
            return regionService.findAllWhereCountryIsNull().collectList();
        }
     ...
     */
    @GetMapping("/_search")
    public Mono<List<Region>> searchRegions(@RequestParam("query") String query) {
